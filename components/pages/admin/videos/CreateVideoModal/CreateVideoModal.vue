...
<script>
  import CreateVideoMutation from "./createVideoMutation.graphql";
  ...
  export default {
    ...
    methods: {
      async submit() {
        ...
        const { data: { createVideo: video } } = await this.$apollo.mutate({
          mutation: CreateVideoMutation,
          variables: {
            videoIDOrURL: this.videoIDOrURL.transformedValue
          }
        });
