...
<script>
  import UploadedFileByAliasQuery from "./uploadedFileByAliasQuery.graphql";
  import UploadedFileQuery from "./uploadedFileQuery.graphql";
  import UpdateUploadedFileMutation from "./updateUploadedFileMutation.graphql";
  import DeleteUploadedFileMutation from "./deleteUploadedFileMutation.graphql";
  ...
  export default {
        ...
        fields: {
          ...
          alias: new InputFieldCompanion({
            ...
            validateOrSaveAsync: async value => {
              const { data: { uploadedFileByAlias: uploadedFile } } = await this.$apollo.query({
                query: UploadedFileByAliasQuery,
                variables: {
                  alias: value
                }
              });
    ...
    methods: {
      async onActivate() {
        ...
        this.loading = true;
        const { data: { uploadedFile } } = await this.$apollo.query({
      ...
      save() {
        ...
        this.$apollo.mutate({
          mutation: UpdateUploadedFileMutation,
          variables: {
            id: this.savedUploadedFile.id,
            uploadedFile: {
              title: this.fields.title.changed ? this.fields.title.transformedValue : undefined,
              alias
            }
          }
        });
